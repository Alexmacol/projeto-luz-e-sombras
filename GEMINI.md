...
- **JavaScript (ES Modules):**
  ...
  - **Modularização:** Separação de responsabilidades em módulos específicos (detalhes na estrutura de diretórios).
...
└── src\
    ...
    ├── js\
    │   ├── main.js         # Controlador principal, inicialização e gestão de eventos
    │   ├── api.js          # Camada de serviço para abstração da busca de dados
    │   ├── ui.js           # Manipulação do DOM e renderização (View)
    │   ├── timeline.js     # Lógica da linha do tempo circular
    │   ├── search.js       # Estrutura de busca e filtro (Pendente de integração)
